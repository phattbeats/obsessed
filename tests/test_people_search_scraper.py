...
LISTING_FIXTURE = FIXTURE_DIR / "fastpeoplesearch_jordan_rivera_oh.html"
DETAIL_FIXTURE = FIXTURE_DIR / "fps_jordan_rivera_detail.html"
...
def test_parse_listing_people_extracts_person_records():
    ...
    first = people[0]
    assert first["name"].lower().startswith("jordan")
...
def test_build_search_url_includes_city_and_state():
    url = people_search._build_search_url("Jordan", "Rivera", state="OH", city="Hilliard")
    assert url.endswith("/name/jordan-rivera_hilliard-oh")
...
def test_search_people_uses_flaresolverr_then_parses_listing():
    ...
    with patch.object(people_search, "fs_get", fs_get_mock):
        people = asyncio.run(
            people_search.search_people("Jordan", "Rivera", state="OH", use_flaresolverr=True)
        )
    assert fs_get_mock.await_count == 1
    called_url = fs_get_mock.await_args.args[0]
    assert called_url == "https://www.fastpeoplesearch.com/name/jordan-rivera_oh"
    assert people and people[0]["name"].lower().startswith("jordan")
...
<script type="application/ld+json">
{
  ...
  "@type": "Person",
  "@id": "https://www.fastpeoplesearch.com/jordan-rivera_id_G1000000000000000001",
  "url": "https://www.fastpeoplesearch.com/jordan-rivera_id_G1000000000000000001",
  "name": "Jordan Rivera",
  "HomeLocation": [{
    "@type": "Place",
...
def test_get_person_detail_solves_turnstile_then_reissues():
    ...
    with patch.object(people_search, "fs_get", fs_get_mock), \
        ...
        result = asyncio.run(
            people_search.get_person_detail(
                "https://www.fastpeoplesearch.com/jordan-rivera_id_G1000000000000000001",
    ...
    assert page_url_arg.endswith("G1000000000000000001")
    ...
    assert result["person"]["name"] == "Jordan Rivera"
...
def test_get_person_detail_skips_captcha_gracefully_when_key_empty():
    ...
    with patch.object(people_search, "fs_get", fs_get_mock), \
        ...
        result = asyncio.run(
            people_search.get_person_detail(
                "https://www.fastpeoplesearch.com/jordan-rivera_id_X",
                use_flaresolverr=True,
                use_captcha=True,
            )
        )
...
def test_get_person_detail_skips_captcha_when_use_captcha_false():
    ...
    with patch.object(people_search, "fs_get", fs_get_mock), \
        ...
        result = asyncio.run(
            people_search.get_person_detail(
                "https://www.fastpeoplesearch.com/jordan-rivera_id_Y",
                use_flaresolverr=True,
                use_captcha=False,
            )
        )
...
def test_get_person_detail_no_challenge_parses_directly():
    ...
    with patch.object(people_search, "fs_get", fs_get_mock), \
        ...
        result = asyncio.run(
            people_search.get_person_detail(
                "https://www.fastpeoplesearch.com/jordan-rivera_id_Z",
                use_flaresolverr=True,
                use_captcha=True,
            )
        )
