...
async def ohio_sos(client: httpx.AsyncClient) -> list[Probe]:
    ...
    api_calls = [
        ("ohio_sos_byname_jordan_rivera",
         "https://businesssearch.ohiosos.gov/?=businessdetails/Name%2C%20Jordan%20Rivera"),
        ("ohio_sos_search_jordan_rivera",
         "https://bizimage.ohiosos.gov/api/?nameTermStartsOrContains=contains&name=rivera+jordan"),
        ("ohio_sos_search_rivera_jordan",
         "https://bizimage.ohiosos.gov/api/?nameTermStartsOrContains=contains&name=jordan+rivera"),
    ]
...
async def ohio_voter(client: httpx.AsyncClient) -> list[Probe]:
    ...
    form = {
        **vs,
        ...
        "ctl00$MainContent$txtFirstName": "Jordan",
        "ctl00$MainContent$txtLastName": "Rivera",
        "ctl00$MainContent$btnSearch": "Search",
    }
    p = await probe(client, "ohio_voter_search_jordan_rivera_franklin",
    ...
    if p.ok:
        ...
        for r in rows[:20]:
            ...
            if "Rivera" in txt and len(txt) < 400:
    ...
    out.append(await probe(client, "ohio_voter_search_jordan_rivera_statewide",
                           "https://voterlookup.ohiosos.gov/voterlookup.aspx",
...
async def fastpeoplesearch(client: httpx.AsyncClient) -> list[Probe]:
    out = []
    urls = [
        ("fastpeoplesearch_jordan_rivera_oh",
         "https://www.fastpeoplesearch.com/name/jordan-rivera_oh"),
        ("fastpeoplesearch_jordan_rivera_hilliard",
         "https://www.fastpeoplesearch.com/name/jordan-rivera_hilliard-oh"),
        ("fastpeoplesearch_jordan_rivera_dublin",
         "https://www.fastpeoplesearch.com/name/jordan-rivera_dublin-oh"),
    ]
    for name, url in urls:
        ...
        if p.ok:
            ...
            for card in soup.select(".card-block, .people-list-card, .person-info"):
                ...
                if "Jordan" in txt and "Rivera" in txt:
                    p.findings.append(txt)
            # generic age + city heuristic
            for el in soup.find_all(string=re.compile(r"\b(Jordan .* Rivera|Rivera, Jordan)\b", re.I)):
                t = el.strip()[:250]
...
async def truepeoplesearch(client: httpx.AsyncClient) -> list[Probe]:
    out = []
    urls = [
        ("truepeoplesearch_jordan_rivera_oh",
         "https://www.truepeoplesearch.com/results?name=Jordan%20Rivera&citystatezip=OH"),
        ("truepeoplesearch_jordan_rivera_hilliard",
         "https://www.truepeoplesearch.com/results?name=Jordan%20Rivera&citystatezip=Hilliard%2C%20OH"),
        ("truepeoplesearch_jordan_rivera_franklin",
         "https://www.truepeoplesearch.com/results?name=Jordan%20Rivera&citystatezip=Franklin%20County%2C%20OH"),
    ]
    for name, url in urls:
        p = await probe(client, name, url)
        out.append(p)
...
async def auditor_addresses(client: httpx.AsyncClient) -> list[Probe]:
    """Re-run the auditor street searches against current Jordan Rivera records."""
    out = []
    ...
    p = await probe(client, "franklin_auditor_owner_rivera_jordan",
