...
# NOTE: /api/admin/* auth is enforced once at the router level via
# dependencies=[Depends(require_admin_token)] above. The old per-route
# require_admin() helper was removed — it returned a bare function (not a
# Depends), so FastAPI crashed at import when used in a route's
# dependencies=[...] list (PHA-824 boot fix).
